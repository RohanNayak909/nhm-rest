...
@Repository
public class RestProjectSchemeAbstractReportDao {
	...
	public JsonResponse<List<RestAdminDashboardModel>> projectSchemeAbstractReportViewDao() {
		...
		try {
			List<Object[]> x = em.createNamedStoredProcedureQuery("admin_project_scheme_abstract_report")
					.getResultList();

			for (Object[] m : x) {
				
				RestAdminDashboardModel so = new RestAdminDashboardModel(m[0],m[1],m[2],m[3],m[4],m[5],m[6],m[7],m[8],m[9],m[10],m[11],m[12],m[13],m[14],m[15],m[16],m[17],m[18]);
				admin.add(so);
