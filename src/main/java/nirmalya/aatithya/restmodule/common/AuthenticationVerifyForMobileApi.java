...
public class AuthenticationVerifyForMobileApi {
	 ...
	 * em.createNamedStoredProcedureQuery("loginRestApi")
	 * .setParameter("actionType", "authVerification").setParameter("actionValue",
	 * value).getResultList(); if (!x.isEmpty()) { for (Object[] m : x) {
	 * 
	 * LoginDataModel employement = new LoginDataModel(m[0], m[1], null, null);
	 * employementListValid.add(employement);
	 * 
	 * } if (employementListValid.get(0).getUserId().equals(loginId)) { return true;
