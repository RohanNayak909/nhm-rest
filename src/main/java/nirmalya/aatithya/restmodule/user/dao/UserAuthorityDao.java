...
public class UserAuthorityDao {
	 ...
	 * getAllSetAuthority(DataTableRequest request) {
	 * logger.info("Method : getAllSetAuthority starts"); List<UserSetAuthority>
	 * form = new ArrayList<UserSetAuthority>();
	 * 
	 ...
	 * em.createNamedStoredProcedureQuery("UserSetAuthority")
	 * .setParameter("actionType", "viewAllAuthority").setParameter("actionValue",
	 * values).getResultList();
	 * 
	 * for (Object[] m : x) {
	 * 
	 * UserSetAuthority setAuthority = new UserSetAuthority(m[0], m[1], m[2], null,
	 * m[3], m[4], m[5], null); form.add(setAuthority); }
	 * 
	 * if (x.get(0).length > 6) { BigInteger t = (BigInteger) x.get(0)[6];
	 * 
	 * total = Integer.parseInt((t.toString())); }
	 * 
	 * } catch (Exception e) { // TODO Auto-generated catch block
	 * e.printStackTrace(); } JsonResponse<List<UserSetAuthority>> resp = new
	 * JsonResponse<List<UserSetAuthority>>(); resp.setBody(form);
	 * resp.setTotal(total);
	 * 
	 * HttpHeaders responseHeaders = new HttpHeaders();
	 * responseHeaders.set("MyResponseHeader", "MyValue");
	 * ResponseEntity<JsonResponse<List<UserSetAuthority>>> response = new
	 * ResponseEntity<JsonResponse<List<UserSetAuthority>>>( resp, responseHeaders,
	 * HttpStatus.CREATED); logger.info("Method : getAlluserAuthority end");
	 * 
	 * return response; }
	 */
