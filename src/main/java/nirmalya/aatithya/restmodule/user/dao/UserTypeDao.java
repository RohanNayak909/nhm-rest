...
public class UserTypeDao {
	 ...
	 */
	/*
	 * public ResponseEntity<JsonResponse<List<UserTypeModel>>>
	 * getAllUserTypes(DataTableRequest request) {
	 * logger.info("Method : getAllUserTypes starts"); List<UserTypeModel> form =
	 * new ArrayList<UserTypeModel>();
	 * 
	 * String values = GenerateParameter.getSearchParam(request); Integer total = 0;
	 * try {
	 * 
	 * @SuppressWarnings("unchecked") List<Object[]> x =
	 * em.createNamedStoredProcedureQuery("UserType") .setParameter("actionType",
	 * "viewAlluserType").setParameter("actionValue", values).getResultList();
	 * 
	 * if (!x.isEmpty()) { for (Object[] m : x) { UserTypeModel userType = new
	 * UserTypeModel(m[0], m[1], m[2], m[3]); form.add(userType); }
	 * 
	 * if (x.get(0).length > 4) { BigInteger t = (BigInteger) x.get(0)[4];
	 * 
	 * total = Integer.parseInt((t.toString())); } } } catch (Exception e) { // TODO
	 * Auto-generated catch block e.printStackTrace(); }
	 * JsonResponse<List<UserTypeModel>> resp = new
	 * JsonResponse<List<UserTypeModel>>(); resp.setBody(form);
	 * resp.setTotal(total);
	 * 
	 * HttpHeaders responseHeaders = new HttpHeaders();
	 * responseHeaders.set("MyResponseHeader", "MyValue");
	 * ResponseEntity<JsonResponse<List<UserTypeModel>>> response = new
	 * ResponseEntity<JsonResponse<List<UserTypeModel>>>( resp, responseHeaders,
	 * HttpStatus.CREATED); logger.info("Method : getAllUserTypes end"); return
	 * response; }
	 */
